...
Instead of `ENB` you may use `bem-tools` project builder. The result file structure and paths to configuration files are the same in both cases, though `ENB` is more rapid and flexible.
...
You need to restart a server after changing the configuration to apply all changes. Kill the current process (`Ctrl+C`) and run server again.
...
While developing every time you reload a page in a browser a server mode rebuilds what has to be rebuilt following your changes.

To rebuild the entire project you can use following `ENB` command:
