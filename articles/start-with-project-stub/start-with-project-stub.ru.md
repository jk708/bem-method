...
Самый оптимальный путь создания собственного проекта – использование шаблонного репозитория [project-stub](https://github.com/mdevils/project-stub). Он содержит необходимый минимум конфигурационных файлов и папок и позволяет быстро развернуть проект.

Для создания тестового проекта запустим сборщик [ENB](http://enb-make.info/) в режиме сервера.
...
Нам понадобится локальная копия `project-stub`. Ее можно сделать любым удобным для вас способом. Мы собираемся использовать Git.
    ...
    $ git clone git://github.com/mdevils/project-stub.git test-project
...
Инициализируем и загружаем подмодули `ENB`:

    $ git submodule init
    $ git submodule update

Устанавливаем `bem-tools` для работы с проектом из командной строки:
...
Конфигурация процесса сборки хранится в файле `.bem/make.js`. На её основе `ENB` подключает все технологии, которые составляют реализацию блоков: шаблоны, зависимости, CSS-правила и JavaScript-функциональность.
...
На вашем компьютере запустился `bem server` — инструмент для разработки, который при обновлении страницы в браузере будет автоматически пересобирать только ту часть проекта, которую затронули ваши изменения. Результат доступен по ссылке: [http://localhost:8080/desktop.bundles/index/index.html](http://localhost:8080/desktop.bundles/index/index.html)

Альтернативой `ENB` может служить встроенный сборщик [bem-tools](http://ru.bem.info/tools/bem/bem-tools/) - более простой в использовании, однако менее гибкий инструмент. Результаты сборки и путь к конфигурационным файлам в обоих случаях одинаковы.

**Важно**: В данном тестовом примере мы используем различные версии `project-stub` для сборки средствами `ENB` и `bem-tools`. Каждый шаблонный проект заранее настроен под соответствующий сборщик.

Ход создания проекта с `bem-tools` немногим отличается от описанного выше:
...
Всё то время, пока мы разрабатывали проект, работал `bem server`, который пересобирал только изменённые части проекта, необходимые при обновлении страницы в браузере.
